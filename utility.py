...
def define_simulation_graph(batch_env, algo_cls, config):
  ...
  # locals() is a dictionary, and then it is wrapped by AttrDict() to be a class, turning the keys as attributes (@property).
...
def define_batch_env_multi(*constructor, num_agents, env_processes):
  """Create environments and apply all desired wrappers.
  ...
  with tf.variable_scope('environments'):
    ...
    if env_processes:
      multi = len(constructor)
     # print('multi is:',multi)
      #print('num_agents is:',num_agents)
      multi_agents = []
      for i in range(multi):
        multi_agents.append(num_agents//multi)
        #print('multi_agents[',i,'] is:',multi_agents[i])
      if num_agents % multi != 0:
        multi_agents[0] = num_agents % multi + multi_agents[0]

      multi_agents[0]=5
      multi_agents[1]=25

      envs=[]
      for j in range(len(multi_agents)):
        env = [tools.wrappers.ExternalProcess(constructor[j]) for _ in range(multi_agents[j])]
        envs=envs+env
    else:
      envs = [constructor() for _ in range(num_agents)]
    batch_env = tools.BatchEnv(envs, blocking=not env_processes)
